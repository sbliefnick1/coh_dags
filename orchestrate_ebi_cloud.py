...
with DAG('orchestrate_ebi_cloud', default_args=default_args, catchup=False, schedule_interval='0 3 * * *') as dag:
    ...
    aai_dbt_build = SSHOperator(
        ssh_conn_id='ebi_etl_server',
        task_id='aai_dbt_build',
        command=f'{prefix} aai_dbt_build.py',
    )

    refresh_tableau_cloud_extracts = SSHOperator(
        ssh_conn_id='ebi_etl_server',
        task_id='refresh_tableau_cloud_extracts',
        command=f'{prefix} refresh_tableau_cloud_extracts.py',
    )

    refresh_cfin_tableau_cloud_extracts = SSHOperator(
        ssh_conn_id='ebi_etl_server',
        task_id='refresh_cfin_tableau_cloud_extracts',
        command=f'{prefix} refresh_cfin_tableau_cloud_extracts.py',
    )
